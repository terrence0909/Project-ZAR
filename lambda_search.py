...
# ========== NEW FUNCTION: GET ALL CUSTOMERS ==========
def get_all_customers():
    """Return ALL customers for GET /customers endpoint"""
    try:
        ...
        # Get all wallets
        wallets_response = wallets_table.scan()
        all_wallets = wallets_response.get('Items', [])
        
        # Group wallets by customer
        customer_wallets = {}
        for wallet in all_wallets:
            cust_id = wallet.get('customer_id')
            if cust_id:
                if cust_id not in customer_wallets:
                    customer_wallets[cust_id] = []
                customer_wallets[cust_id].append(wallet)
        
        # Format customers
        formatted_customers = []
        for customer in customers:
            cust_id = customer.get('customer_id')
            wallets_list = customer_wallets.get(cust_id, [])
            
            # Calculate average risk
            total_risk = sum(float(w.get('risk_score', 0)) for w in wallets_list)
            avg_risk = total_risk / len(wallets_list) if wallets_list else 0
            
            # Get primary wallet (first declared or first wallet)
            primary_wallet = None
            for wallet in wallets_list:
                if wallet.get('declared', False):
                    primary_wallet = wallet
                    break
            if not primary_wallet and wallets_list:
                primary_wallet = wallets_list[0]
            ...
            formatted_customers.append({
                ...
                'vasp_id': customer.get('vasp_id', ''),
                'created_at': customer.get('created_at', datetime.now().isoformat()),
                'wallet_count': len(wallets_list),
                'average_risk_score': round(avg_risk, 1),
                'highest_risk_score': max([float(w.get('risk_score', 0)) for w in wallets_list], default=0),
                'total_balance': sum(float(w.get('balance', 0)) for w in wallets_list),
                'primary_wallet': primary_wallet.get('wallet_address', '') if primary_wallet else '',
                'primary_wallet_risk': float(primary_wallet.get('risk_score', 0)) if primary_wallet else 0,
                'wallets': [
                    {
                        'address': w.get('wallet_address', ''),
                        'risk_score': float(w.get('risk_score', 0)),
                        'declared': w.get('declared', False),
        ...
        formatted_customers.sort(key=lambda x: x['highest_risk_score'], reverse=True)
...
def enrich_with_luno(wallet_address, customer_profile='default', wallet_balances=None):
    ...
    try:
        ...
        print(f"=== DEBUG: enrich_with_luno ===")
        print(f"Wallet: {wallet_address}")
        print(f"Profile: {customer_profile}")
        print(f"Balances: {wallet_balances}")
        
        print(f"Calling Luno Lambda: project-zar-luno-enrich")
        ...
        print(f"Luno Lambda response status: {response['StatusCode']}")
        
        result = json.loads(response['Payload'].read().decode())
        print(f"Raw Luno response: {json.dumps(result)[:200]}...")
        
        body = json.loads(result['body'])
        luno_data = body.get('luno_data')
        
        if luno_data:
            ticker_count = len(luno_data.get('market_tickers', []))
            print(f"✓ Luno data retrieved: {ticker_count} tickers")
            if ticker_count > 0:
                print(f"First few tickers: {[t['pair'] for t in luno_data['market_tickers'][:3]]}")
...
def lambda_handler(event, context):
    try:
        print("=== SEARCH LAMBDA START ===")
        print("Raw event received:", json.dumps(event))
        
        # ========== NEW: Check if this is a GET request to /customers ==========
        http_method = event.get('httpMethod', 'POST')
        path = event.get('path', '')
        resource = event.get('resource', '')
        
        print(f"HTTP Method: {http_method}, Path: {path}, Resource: {resource}")
        
        # Handle GET /customers request
        if http_method == 'GET' and ('/customers' in path or '/customers' in resource):
            print("Detected GET /customers request - returning all customers")
        ...
        # ========== NEW: Also handle POST /customers with get_customers query_type ==========
        if query_type == 'get_customers' or (http_method == 'POST' and '/customers' in path and not query):
            print("Detected POST /customers with get_customers query_type")
            return get_all_customers()
        # ====================================================================================
